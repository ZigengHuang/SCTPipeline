...
class Config:
    def __init__(self):
        ...
        parser.add_argument(
            "-k", "--knowledge",
            required=True,
            help="Medical knowledge base path"
        )
        ...
        self.knowledge_base = os.path.normpath(args.knowledge)

        # Find SCT.txt
        sct_file = Path(self.knowledge_base) / "SCT.txt"
        if not sct_file.exists():
            raise FileNotFoundError(f"SCT.txt not found in {self.knowledge_base}.")
        self.sct_file = str(sct_file)

        # Processing parameters
        self.vocab_size = 1000
        self.similarity_threshold = 0.9

        # Choose medical domains
        self.medical_domain = self.select_medical_domain()

    def select_medical_domain(self) -> str:
        """Choose the predefined prompt for medical domains"""
        domain_options = {
            1: "Health checkups",
            2: "Routine outpatient guidance",
            3: "Surgery",
            4: "Hospitalization guidance",
            5: "Customized domain-specific features"  
        }
        
        print("\n" + "="*30)
        print("Please choose the medical domain：")
        for num, domain in domain_options.items():
            print(f"{num}. {domain}")
        ...
        while True:
            ...
            if not choice:
                return domain_options[1]
            try:
                choice_num = int(choice)
                if choice_num == 5:
                    return self._generate_custom_domain(self.sct_file)
                elif choice_num in domain_options:
                    return domain_options[choice_num]
                print(f"Please enter a valid number between 1-5.")
            except ValueError:
...
class MedicalTranscriptionPipeline:
    def __init__(self, config: Config):
        ...
        self.client = OpenAI(
            ...
            base_url="https://api.chatanywhere.tech/v1" # https://api.chatanywhere.tech/v1/chat/completions
        )
    ...
    def get_domain_prompt(self) -> str:
        ...
        if self.cfg.medical_domain.startswith("Customized domain-specific features"):
            ...
            domain_mapping = {
                ...
                "Routine outpatient guidance": "Routine outpatient guidance",
                "Surgery": "Surgery",
                "Hospitalization guidance": "Hospitalization guidance"
            }
            
            domain_key = domain_mapping.get(self.cfg.medical_domain)
            if not domain_key:
                raise ValueError(f"Invalid medical domain：{self.cfg.medical_domain}")
            
            if domain_data := self.domain_prompts.get(domain_key):
                return self._format_domain_prompt(domain_data)
            
            raise ValueError(f"The medical domain cannot be found in the configuration file：{domain_key}")
    ...
    def load_knowledge(self) -> str:
        ...
        if missing:
            raise FileNotFoundError(f"Missing knowledge base files: {missing}")
...
[Complete medical conversation transcriptions]"""
    ...
    # ====================
    # Phase 4: Generation of SCT (Steps 25-28)
    # ====================
    ...
    def run_pipeline(self):
        ...
        try:
            ...
            if not self._user_continue():  # 如果用户选择不继续
